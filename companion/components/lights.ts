...
export class Lights {
    ...
    initialize() {
        this.app.broker.registerHandler(
            Commands.LIGHTS_UPDATE_GROUPLIST,
            async () => {
                ...
                this.app.state.lights.groupsData = groups.map(function (group) {
                    return {
                        title: group._data.name,
                        subtitle:
                            'all: ' +
                            group._data.state.all_on +
                            ', any: ' +
                            group._data.state.any_on,
                        _id: group._data.id,
                    };
                });
