...
export class Thermostat {
    ...
    initialize() {
        this.app.broker.registerHandler(
            Commands.THERMOSTAT_UPDATE_ZONESLIST,
            async () => {
                console.log(
                    'companion command THERMOSTAT_UPDATE_ZONESLIST received'
                );
                this.app.resetState();
                this.app.state.page = 'thermostat';
                const zones = await this.api.getZones();
                this.app.state.thermostat.zonesData = zones.map(function (
                    zone
                ) {
                    return {
                        title: zone.name,
        ...
        this.app.broker.registerHandler(
            Commands.THERMOSTAT_SET_TEMPERATURE,
            async (_id, temperature) => {
