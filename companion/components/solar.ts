...
export class Solar {
  ...
  initialize() {
    this.app.broker.registerHandler(Commands.SOLAR_UPDATE_OUTPUTSLIST, async () => {
      console.log('companion command SOLAR_UPDATE_OUTPUTSLIST received');
      this.app.resetState();
      this.app.state.page = 'solar';
      const results = await this.api.getOutputs();
      this.app.state.solar.outputs = results.slice(0,7).map(function (item) {
        return {
          title: item.date,
