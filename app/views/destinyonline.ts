  ...
  ***************************************************************************************************/
  VTList.delegate = {
    ...
    configureTile: function (tile, info) {
      ...
      mixedtext.text = state.destiny.clanmemberPresence[info.index].title;
      mixedtextBody.text = state.destiny.clanmemberPresence[info.index].subtitle;

    },
  };

  // KNOWN ISSUE: It is currently required that VTList.length be set AFTER VTList.delegate
  VTList.length = state.destiny.clanmemberPresence.length;
}
...
export function init(_views: Views) {
  console.log("destinyonline init()");
  views = _views;
  ...
  views.broker.sendCommand(Commands.DESTINY_UPDATE_CLANMEMBERPRESENCE);
}