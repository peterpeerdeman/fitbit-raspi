  ...
  ***************************************************************************************************/
  VTList.delegate = {
    ...
    configureTile: function (tile, info) {
      ...
      mixedtext.text = state.cluster.portTable[info.index].title;
      mixedtextBody.text = state.cluster.portTable[info.index].subtitle;

    },
  };

  // KNOWN ISSUE: It is currently required that VTList.length be set AFTER VTList.delegate
  VTList.length = state.cluster.portTable.length;
}
